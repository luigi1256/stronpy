...
def timeline_created(list_new):
  ...
  if db_note_list!=[]:
   for new_x in list_new:
    if block_word(new_x)==None and  find_japanese(new_x['content'])==[]:  
...
def show_noted():
  ...
  if db_note_list!=[] and len(list_note_out)!=len(db_note_list):
   ...
   button_frame.place(relx=0.5,rely=0.75,relwidth=0.1)      
...
def relay_class():
          if entry_relay.get()!="":
            if entry_relay.get()[0:6]=="wss://" and entry_relay.get()[-1]=="/":
                ...
                counter_relay.place(relx=0.25,rely=0.05)
            else:
...
def show_from_id(List_note):
       items=[]
       for note in List_note:
        replay=nota_reply_id(note)
        for note_x in db_note_list:
           if note_x["id"] in replay:
             if note_x not in items:
              items.append(note_x)
              number=replay.index(note_x["id"])
              replay.pop(number)
   ...
   if list_note_save!=[]: 
    ...
    def create_note(note_text, s):
        ...
        if note_text["content"] not in list_note_lib:
         ...
         button_grid3=Button(scrollable_frame,text=f"Photo", command=lambda val=note: print_ph_oto(val))
