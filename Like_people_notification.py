...
combo_box.place(relx=0.05,rely=0.09)
...
label_entry_id.place(relx=0.05,rely=0.13)
...
combo_note = ttk.Combobox(root, values=["Total","My In","Inbox","More Like","Open reaction","my time","Classification"],width=13, font=("Arial",12,"normal"))
combo_note.place(relx=0.22,rely=0.08)
...
def on_reaction(event):
    selected_item = combo_reaction.get()
    if selected_item in list(like_d_reaction.keys()):
       show_Teed()

def close_reaction():
   combo_reaction.set("")
   combo_reaction.place_forget()
   label_reaction.place_forget()
   button_close_reaction.place_forget()

button_close_reaction=tk.Button(root,text="🗙",command=close_reaction, font=('Arial',12,'bold'),foreground="red")
combo_reaction = ttk.Combobox(root, values=[],font=('Arial',12,'normal'),width=10)
combo_reaction.set("")
combo_reaction.bind("<<ComboboxSelected>>", on_reaction)
label_reaction=Label(root,text="Reaction",font=('Arial',12,'normal'))

classification_like={}

def new_value(event):
    select_item=combo_like.get()
    if int(select_item)>1:
     list_event=like_classification(db_list,int(select_item))
     if list_event:
        widget_list(list_event)

combo_like = ttk.Combobox(root, values=[],font=('Arial',12,'normal'),width=5)
combo_like.set(2)
combo_like.bind("<<ComboboxSelected>>", new_value)

def like_classification(kind_7:list,value_like:int):
    list_like=[]
    db_d_list=[]
    if kind_7!=[]:
     if classification_like=={}: 
      for x_like in kind_7:
        if tags_string(x_like,'e')!=[]:
            str_like=tags_string(x_like,'e')[0]
            if str_like not in list_like:
                classification_like[str_like]=int(1)
                list_like.append(str_like)
            else:
              if str_like in list(classification_like.keys()):
                    number=classification_like[str_like]
                    number =int(number)+1
                    classification_like[str_like]=int(number)
     like_set=set(classification_like.values())
     combo_like["values"]==[]
     combo_like["values"]=list(like_set)
     combo_like.place(relx=0.63,rely=0.4,relheight=0.04)      
     for key,value in classification_like.items():
         if value>=value_like:
            db_d_list.append(key)
     list_note=get_note(asyncio.run(Get_event_id(db_d_list)))         
     if list_note!=[]:
         return list_note

def like_note_seven(kind_7:list):
...
def list_timeline(Value):
  if Value!="Type of feed":  
   if Value in combo_note["values"]:
      ...
      if Value=="More Like": 
         More_Like.clear()
         More_Like_id=like_note_seven(db_list)
         if More_Like_id!=None:   
           
           More_Like.extend(More_Like_id)
           return More_Like
      if Value=="Open reaction":
         like_reaction.clear()
         button_close_reaction.place(relx=0.47,rely=0.1)
         combo_reaction.place(relx=0.38,rely=0.15 )
         label_reaction.place(relx=0.38,rely=0.11 )
         if like_d_reaction=={}:
            for db_x in db_list:
                if db_x["kind"]==7:
                    if db_x["content"] in list(like_d_reaction.keys()):
                        list_rea=like_d_reaction[db_x["content"]]
                        list_rea.append(db_x)
                        like_d_reaction[db_x["content"]]=list_rea
                    else:
                        like_d_reaction[db_x["content"]]=[db_x]   
            combo_reaction["values"]=list(like_d_reaction.keys())
         else:
           try: 
            list_reaction=like_d_reaction[combo_reaction.get()]
            like_reaction.extend(list_reaction)
      ...
      if Value=="Classification":
        if Classification_list==[]:
         test_result=like_classification(db_list,int(combo_like.get()))       
         if test_result:
            Classification_list.extend(test_result)
        widget_list(Classification_list) 
...
Button_check_2.place(relx=0.15,rely=0.22)

In_out=IntVar()
Button_check_3 = Checkbutton(root, text ="My like" , variable = In_out, onvalue = 1, offvalue = 0, height = 1,font=('Arial',12,'bold'))
Button_check_3.place(relx=0.25,rely=0.27)

async def get_note_text(client, user):
  try:
   if WoT_check.get()==1:
    f = Filter().authors(user).kinds([Kind(7)]).pubkey(PublicKey.parse(my_dict[combo_box.get()])).limit(200)
   else:
    if In_out.get()==1:
     f = Filter().kinds([Kind(7)]).author(PublicKey.parse(my_dict[combo_box.get()])).limit(80)   
     print(f)
    else:
        f = Filter().kinds([Kind(7)]).pubkey(PublicKey.parse(my_dict[combo_box.get()])).limit(200)   
   events = await Client.fetch_events(client,f,timeout=timedelta(seconds=10))  
   z = [event.as_json() for event in events.to_vec() if event.verify()]
   return z
...
def show_Teed():
 ...
 def create_page(db_list_,s):
  if db_list_!=[] and db_list_!=None:
    ...
    for note in db_list_[n:l]:
     try:
      if note["pubkey"] in list(user_metadata.keys()):
            context0="Nickname " +user_metadata[note["pubkey"]][0:15]
      else:
            if note["pubkey"] in my_list:
             for user_0 in my_name:
              if my_dict[user_0]==note["pubkey"]:
               context0="Nickname " +user_0[0:15]
            else: 
      ...
      if tags_string(note,"e")!=[]:
        if four_tags(note,"e"):
            for F_note in four_tags(note,"e"):
                if len(F_note)>3:
                 if len(F_note[3])==64:
                    context2=context2+str(" < "+ F_note[0]+" > ")+str(F_note[1][0:9])+ "\n"
                 else:
                    context2=context2+str(" < "+ F_note[0]+" > ")+str(F_note[3])+ "\n"
                else:
                 context2=context2+str(" < "+ F_note[0]+" > " + " NO NIP-10"+ "\n")  
      if tags_string(note,"k")!=[]:
                context2=context2+"- K "+str(tags_string(note,"k")[0]) +"\n"
      if tags_string(note,"p")!=[]:
          context2=context2+str(len(tags_string(note,"p"))) +" p Tag "+"\n"+"- p "+str(tags_string(note,"p")[0]) +"\n"          
      ...
      def print_photo(entry):
            if tags_str(entry,"emoji")!=[]:
               print(tags_str(entry,"emoji")[0][2])   
      ...
      if tags_str(note,"emoji")!=[]:
       button_grid2=Button(scrollable_frame_1,text=f"Photo ", command=lambda val=note: print_photo(val))
       button_grid2.grid(row=s+3,column=2,padx=5,pady=5)    
      s=s+4  
    ...
    frame2.place(relx=0.18,rely=0.45,relwidth=0.2,relheight=0.4)
    ...
    button_next.place(relx=0.28,rely=0.4, anchor="n")
    button_back.place(relx=0.23,rely=0.4, anchor="n",x=1)
    lbel_var.place(relx=0.18,rely=0.4, anchor="n",relwidth=0.02,relheight=0.04)
    ...
    button_f_close.place(relx=0.33,rely=0.4)      
    button_frame=Button(root,command=close_frame,text="Close ❌",font=("Arial",12,"normal"))
    button_frame.place(relx=0.22,rely=0.9,relwidth=0.1)  
...
async def get_outbox(client):
  ...
  if my_list!=[]:
   if my_dict[combo_box.get()] in list(my_dict.values()): 
    ...
    f = Filter().authors(user_convert([my_dict[combo_box.get()]])).kinds([Kind(10002)])
    events = await Client.fetch_events(client,f,timeout=timedelta(seconds=10))  
    z = [event.as_json() for event in events.to_vec() if event.verify()]
    return z
...
button4.place(relx=0.05,rely=0.27) 
...
def metadata_0(nota,y):
   ...
   try:
    ...
    if y in list(test.keys()) and test[y]!="":
     return str(test[y])
   except KeyError as e:
      print(e)
...
def open_new_user():
 if len(my_dict)==5:
  if Check_open_2.get()==1:
   Check_open_2.set(0)
  frame_user.place(relx=0.02,rely=0.37,relheight=0.27,relwidth=0.28)
...
numb_close.grid(column=0,row=0,pady=5,padx=5)
...
label_pubkey.grid(column=0,row=3,pady=2,padx=10)
user_name=StringVar()
...
def show_print_test_tag(note):
   ...
   if note["pubkey"] in list(user_metadata.keys()):
              context00="Nickname " +user_metadata[note["pubkey"]][0:15]
   else:
                context00="Pubkey "+note["pubkey"][0:9]
           ...
           if jresult["id"]!=entry["id"]:  
             ...
             if jresult["pubkey"] in list(user_metadata.keys()):
              context0="Nickname " +str(user_metadata[jresult["pubkey"]])
                ...
                context0="Pubkey "+jresult["pubkey"][0:9]
             ...
             if tags_string(jresult,"e")!=[]:
              if four_tags(jresult,"e"):
                for F_note in four_tags(jresult,"e"):
                     if len(F_note)>3:
                       if len(F_note[3])==64:
                        context22=context22+str(" < "+ F_note[0]+" > ")+str(F_note[1][0:9])+ "\n"
                       else:
                        context22=context22+str(" < "+ F_note[0]+" > ")+str(F_note[3])+ "\n"
                     else:
                        context22=context22+str(" < "+ F_note[0]+" > " + " NO NIP-10"+ "\n")  
   ...
   if tags_string(note,"e")!=[]:
    button_grid3=Button(scrollable_frame_2,text=f"Reaction Note ", command=lambda val=note: print_content(val))
      ...
      if note["kind"]==7:
         if note["created_at"]>int(float(time.time()-604800)):
            if note not in zap_list:
               ...
               if tags_string(note,"e")[0] not in note_list:
                note_list.append(tags_string(note,"e")[0])

   if zap_list!=[]:
      var_id_amount.set("")
      s=len(note_list)
      if s>0:          
        print("for ",str(s), "note liked ",len(zap_list))   
        var_id_amount.set("for "+str(s)+ " note  receveid "+ str(len(zap_list)) +" like ") 
...
def Count_likes_Day():
   ...
   if zap_list!=[]:
      s=len(note_list)
      if s>0:          
        print("for ",str(s), "note receveid",str(len(zap_list))," likes ",)   
        var_id_amount_time.set("for "+str(s)+ " note receveid "+str(len(zap_list)) +" likes ") 
...
time_pm = ttk.LabelFrame(root, text="Likes in Days", labelanchor="n", padding=10)
time_am.place(relx=0.51,rely=0.15,relwidth=0.13,relheight=0.2)
button_count=Button(root,command=count_like,text="Count likes",font=("Arial",12,"normal"))
button_count.place(relx=0.54,rely=0.18) 
button_count_1=Button(root,command=Count_likes_Day,text="Count likes",font=("Arial",12,"normal"))
...
def widget_function():  
   ...
   if db_list!=[]: 
       ...
       for note in db_list[0:100]:
           try:
            ...
            if pubkey in list(user_metadata.keys()):
                var_id_3.set(str(user_metadata[pubkey]) )
            else:
                var_id_3.set(pubkey[0:9])   
            ...
            if note["content"]!="":
                ...
                if str(note["content"]).startswith(":"):
                    var_id_4.set(" emoji "+ note["content"])
                else:
                    var_id_4.set(" Like "+ note["content"])   
    ...
    frame3.place(relx=0.4,rely=0.5,relwidth=0.55)      
...
button_close_=tk.Button(root,text="Like List",command=widget_function, font=('Arial',12,'bold'))
button_close_.place(relx=0.55,rely=0.4)       

def widget_list(list_kind:list):  
    """Widget function \n
    View of note
    """
    if list_kind!=[]: 
        ...
        for note in list_kind[0:100]:
           try:
            ...
            if pubkey in list(user_metadata.keys()):
                ...
                if pubkey in my_list:
                    for user_0 in my_name:
                        if my_dict[user_0]==pubkey:
                            var_id_3.set("Nickname " +user_0[0:15])
                else:        
                    var_id_3.set(pubkey[0:9])   
            ...
            if note["content"]!="":
                ...
                var_id_4.set(note["content"][0:30])
                label_id_4.grid(pady=5,column=3,row=s, columnspan=2)
            var_id_5=StringVar()
            label_id_5 = Message(scrollable_frame,textvariable=var_id_5, relief=RAISED,width=200)
            if note["id"] in list(classification_like.keys()):
                var_id_5.set( str(classification_like[note["id"]]) + " like ")
                label_id_5.grid(pady=5,padx=5,column=5,row=s+1)                                                                                                                             
            scroll_bar_mini1 = tk.Scrollbar(scrollable_frame)
            scroll_bar_mini1.grid( sticky = NS,column=4,row=s+1)
            second_label2 = tk.Text(scrollable_frame, padx=8, height=3, width=27, yscrollcommand = scroll_bar_mini1.set, font=('Arial',14,'bold'),background="#D9D6D3")
            context_tags=""
            for note_x in note["tags"]:
               context_tags=context_tags+str(note_x)+"\n"
            second_label2.insert(END,note["content"]+"\n"+str(context_tags))
            scroll_bar_mini1.config( command = second_label2.yview )
            second_label2.grid(padx=10, column=1, row=s+1,pady=5,columnspan=3) 
